"""PAttUNet baseline (Pyramid Attention U-Net) for the benchmark harness.

Upstream repository: https://github.com/faresbougourzi/PDAtt-Unet
Upstream file:       ``Architectures.py`` (repository root), class ``PAttUNet``
Exact revision:      branch ``main2`` at commit
                     ``87a25dc8137eccc3e3b13d4444a705e300b8eec8``

The architecture below is a verbatim copy of the upstream ``PAttUNet`` plus its
...
upstream ``PAttUNet`` never reads it.

Interface::

    model = PAttUNet(n_channels=3, n_classes=2)
    logits = model(x)          # x: [B, 3, H, W] -> [B, 2, H, W] float32

``forward`` returns raw logits (no sigmoid/softmax), matching the other
baselines in this harness. H and W must be divisible by 16 (the pyramid branch
uses ``TF.resize`` with ``divsize=[2, 4, 8, 16]``).
...
class PAttUNet(nn.Module):
    """Pyramid Attention U-Net, upstream ``Architectures.py::PAttUNet``."""
